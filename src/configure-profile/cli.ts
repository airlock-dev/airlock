...
function defaultDecision(): Decision {
  // Profiles are partial — skip by default, user opts tools in
  return 'skip';
}
...
function renderYaml(profileName: string, entries: ToolEntry[]): string {
  const lines: string[] = [`profiles:`, `  ${profileName}:`];

  const renderList = (key: string, decision: 'allow' | 'ask') => {
...
function buildRows(entries: ToolEntry[], serverInfoMap: Map<string, ServerInfo>): Row[] {
  ...
  for (const p of providerOrder) {
    ...
    const counts = { allow: 0, ask: 0, skip: 0 };
...
function renderInspect(entry: ToolEntry, inspectScroll: number): void {
  ...
  out.write(`${DIM}  [j/k] scroll  [i/esc] back  [a] allow  [s] ask  [x] skip${RESET}\n`);
  out.write(`${DIM}  ${'─'.repeat(70)}${RESET}\n\n`);
...
function renderRow(row: Row, isSel: boolean): string[] {
  ...
  if (row.type === 'header') {
    const summary = `${GREEN}${row.allow}✓${RESET} ${YELLOW}${row.ask}?${RESET} ${DIM}${row.skip}⊘${RESET}`;
    ...
    if (isSel) {
      lines.push(`   ${DIM}↳ press a/s/x to set all ${row.total} tools in this provider${RESET}`);
    }
...
function render(
  ...
  const headerLines = [
    '',
    `${BOLD}${CYAN}  Airlock — Configure Profile: ${profileName}${RESET}`,
    `${DIM}  ${'─'.repeat(70)}${RESET}`,
    `${DIM}  [a] allow  [s] ask  [x] skip  [j/k] move  [{/}] section  ${inspectHint}${DIM}  [enter] confirm  [q] quit${RESET}`,
    `${DIM}  on a provider header, a/s/x applies to all tools in that provider${RESET}`,
    `${DIM}  ${'─'.repeat(70)}${RESET}`,
  ];

  const footerScrollParts: string[] = [];
  const included = entries.filter((e) => e.decision !== 'skip').length;
  const summaryLine =
    `${DIM}  ${entries.filter((e) => e.decision === 'allow').length} allow  ` +
    `${entries.filter((e) => e.decision === 'ask').length} ask  ` +
    `${entries.filter((e) => e.decision === 'skip').length} skip  ` +
    `/ ${entries.length} total  (${included} included in profile)${RESET}`;
...
const HELP = `
airlock configure-profile — build allow/ask lists for a reusable profile
...
Options:
  ...
  -p, --profile <name>   Profile name to use in output (default: my-profile)
...
export async function runConfigureProfile(argv: string[]): Promise<void> {
  const { values } = parseArgs({
    args: argv,
    options: {
      config: { type: 'string', short: 'c', default: './airlock.yaml' },
      profile: { type: 'string', short: 'p', default: 'my-profile' },
  ...
  // Pre-populate decisions from existing profile config
  const profileName = values.profile;
  const existingProfile = parsed.profiles[profileName];
  if (existingProfile) {
    const allowSet = new Set(existingProfile.allow);
    const askSet = new Set(existingProfile.ask);
  ...
  render(entries, serverInfoMap, rowIdx, profileName);
  ...
  const yaml = renderYaml(profileName, entries);
