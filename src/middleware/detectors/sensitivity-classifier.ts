...
async function llmScore(
  ...
  const { text: result } = await generateText({
    ...
    model: model as any,
    system:
      'You are a data sensitivity classifier. Rate the sensitivity of the following data on a scale from 0.0 to 1.0, where 0 is public information and 1.0 is highly sensitive (PII, credentials, medical records, financial data). Respond with ONLY a JSON object: {"score": <number>, "reasoning": "<brief explanation>"}',
    prompt: text.slice(0, 20_000),
    maxOutputTokens: 256,
  });

  try {
    return JSON.parse(result) as { score: number; reasoning: string };
  } catch {
...
export function sensitivityClassifierMiddleware(
