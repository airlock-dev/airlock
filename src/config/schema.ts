...
export const ToolArgConstraintConfig = z
  .object({
    ...
    glob_allow: z.array(z.unknown()).optional(),
  ...
  .superRefine((value, ctx) => {
    ...
    const hasGlobAllow = Object.prototype.hasOwnProperty.call(value, 'glob_allow');
    const hasGlobIn = Object.prototype.hasOwnProperty.call(value, 'glob_in');
    const hasEachAllow = Object.prototype.hasOwnProperty.call(value, 'each_allow');
    const hasEachIn = Object.prototype.hasOwnProperty.call(value, 'each_in');
    ...
    if (
      ...
      ctx.addIssue({
        ...
        message:
          'Argument policy constraint must define equals, allow, in, glob_allow, glob_in, each_allow, or each_in.',
      });
    }
    ...
    if (hasGlobAllow && value.glob_allow?.length === 0) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['glob_allow'],
    ...
    if (hasEachAllow && value.each_allow?.length === 0) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['each_allow'],
...
export const ArgScopeConfig = z.record(ToolArgConstraintConfig);
export type ArgScopeConfig = z.infer<typeof ArgScopeConfig>;

export const ValueSetConfig = z.union([
  z.array(z.unknown()),
  z
    .object({
...
export const ArgDimensionConfig = z
  .object({
    bindings: z
      .record(z.union([z.string(), z.array(z.string()).min(1)]))
      .default({})
      .transform((bindings) =>
        Object.fromEntries(
          Object.entries(bindings).map(([tool, args]) => [
            tool,
