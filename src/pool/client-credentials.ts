...
export class ClientCredentialsTokenSource {
  ...
  private usableToken(): string | undefined {
    ...
    if (Date.now() >= this.cached.expires_at - EXPIRY_BUFFER_MS) return undefined;
  ...
  private async mint(): Promise<string> {
    ...
    if (typeof parsed.expires_in === 'number' && parsed.expires_in > 0) {
      token.expires_at = Date.now() + parsed.expires_in * 1000;
    }
  ...
  private async save(): Promise<void> {
    try {
      ...
      await writeFile(this.storePath, JSON.stringify(this.cached, null, 2), { mode: 0o600 });
